        ...
        try (ZipInputStream zis = new ZipInputStream(cl.getResourceAsStream("testfiles.zip"))) {
            ...
            while ((entry = zis.getNextEntry()) != null) {
                if (entry.getName().endsWith(".xlsx")) {
                    XLS xls = new XLS(zis);
                    String actualCellValue = xls.excel.getSheetAt(0).getRow(0).getCell(0).getStringCellValue();
                    Assertions.assertEquals("Name", actualCellValue);
                    break;
                }
            }
        }
